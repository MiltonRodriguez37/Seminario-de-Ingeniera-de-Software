...
class MainWindow(QMainWindow):
    ...
    def click_agregar_inventario(self):
        ...
        cursor.execute("select nombre_producto from inventario where nombre_producto = '{0}'".format(nombre))
        validar_nombre = cursor.fetchall()
        if len(validar_nombre)>0:
            QMessageBox.warning( self, "Error", 'Producto ya existente')
        else:
            try:
                precio = float(precio)
    ...
    def click_modificar_pedido(self):
        numeroPedido = self.ui.lineEdit_buscar_modificar_pedidos.text()
        try:
            cursor.execute("select * from pedidos where numero_pedido = '{0}'".format(int(numeroPedido)))
            validar_numero = cursor.fetchall()
            if len(validar_numero)<1:
                QMessageBox.warning( self, "Error", 'Pedido no encontrado')
            else:
                numeroPedido = int(numeroPedido)
    ...
    def click_modificar_venta(self):
        numeroVenta = self.ui.lineEdit_buscar_modificar_ventas.text()
        try:
            cursor.execute("select * from ventas where numero_venta = '{0}'".format(int(numeroVenta)))
            validar_numero = cursor.fetchall()
            if len(validar_numero)<1:
